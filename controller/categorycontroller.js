...
module.exports.viewCate = async(req,res)=>{
    try {
        ...
        let totalCatedata = await Category.find({
            $or :[
                {"cateName":{$regex : ".*"+search+".*",$options:"i"}},
            ]
        }).countDocuments();
...
module.exports.updateCategory = async(req,res)=>{
    try {
        let adminRecord = await Category.findById(req.params.id);
...
module.exports.editCatData = async(req,res)=>{
    try {
        req.body.updateDate = new Date().toLocaleString();
        let catData = await Category.findByIdAndUpdate(req.body.EditId, req.body);
        console.log(catData);
        if (catData) {
